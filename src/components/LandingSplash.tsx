...
export default function LandingSplash() {
  ...
  useEffect(() => {
    setIsMounted(true);
    // Skip disclaimer if already agreed previously
    if (localStorage.getItem("vidhan-disclaimer-agreed") === "true") {
      setStep("splash");
    }
  }, []);
  ...
  if (!isMounted || step === "done") return null;
    ...
    <AnimatePresence mode="wait">
      {step === "disclaimer" && (
        <motion.div
          ...
          className="fixed inset-0 z-[10000] bg-ink/95 backdrop-blur-sm flex items-center justify-center p-4 md:p-6 overflow-y-auto"
        >
          <div className="bg-white text-ink max-w-3xl w-full rounded-sm shadow-2xl p-8 md:p-12 relative overflow-hidden my-8">
            <h2 className="text-3xl font-serif mb-4">Disclaimer</h2>
            <div className="h-[1px] w-full bg-gold/40 mb-6"></div>
            
            <div className="text-sm md:text-base text-ink/80 leading-relaxed font-sans space-y-4">
              <p>
            ...
            <div className="mt-10 flex justify-end">
              <button
                onClick={() => {
                  localStorage.setItem("vidhan-disclaimer-agreed", "true");
                  setStep("splash");
                }}
                className="bg-ink text-white px-8 py-3 text-sm tracking-widest font-bold uppercase hover:bg-ink/90 transition-colors rounded-sm"
              >
                I Agree
              </button>
            </div>
