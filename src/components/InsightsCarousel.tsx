...
export default function InsightsCarousel({ locale }: { locale: string }) {
  ...
  return (
    <>
      <section ref={sectionRef} className="bg-ink py-24 md:py-0 md:h-screen flex flex-col justify-center overflow-hidden border-t border-gold/10 relative z-10">
        ...
        {activeCard !== null && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 md:p-12 lg:p-24 pointer-events-none">
            ...
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="absolute inset-0 bg-ink/95 backdrop-blur-md pointer-events-auto"
              onClick={() => setActiveCard(null)}
            />

            {/* Expanded Content Modal */}
            <div className="relative z-10 w-full max-w-6xl h-full flex flex-col lg:flex-row bg-ink border border-gold/20 shadow-2xl overflow-hidden pointer-events-auto">
              
              {/* Close Button */}
              <button 
                onClick={() => setActiveCard(null)}
                className="absolute top-4 right-4 lg:top-6 lg:right-6 z-20 text-parchment/50 hover:text-gold transition-colors bg-ink/50 backdrop-blur-md p-2 rounded-full"
              >
                <X size={24} />
              </button>
              ...
              >
                ...
                <div className="absolute inset-0 bg-gradient-to-r from-transparent to-ink/20 pointer-events-none" />
              </motion.div>
              ...
              >
                ...
                <h2 className="text-3xl lg:text-4xl font-serif text-parchment mb-8 leading-tight">
                  {mergedPosts[activeCard].title}
                </h2>
                
                <div className="h-px w-12 bg-gold/50 mb-8" />
                
                <p className="text-base lg:text-lg text-parchment/70 font-sans leading-relaxed mb-6">
                  {mergedPosts[activeCard].description}
                </p>
                
                <a
                  href={`/${locale}/blog`}
                  className="mt-8 group inline-flex items-center gap-3 text-xs tracking-wide-xl uppercase text-gold hover:text-parchment transition-colors w-fit"
                >
