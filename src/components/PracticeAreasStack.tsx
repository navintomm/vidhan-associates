    ...
    <>
      ...
      <div ref={containerRef} className="h-screen w-full bg-ink flex flex-col items-center justify-center relative overflow-hidden pt-24 pb-12">
        ...
        <div className="absolute top-32 left-8 md:left-16 z-20 pointer-events-none">
          ...
          <h1 className="text-4xl md:text-5xl lg:text-7xl font-serif text-parchment leading-tight break-words whitespace-pre-line">
            {t("header.title")}
          </h1>
        </div>
        ...
        <div className="absolute bottom-12 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center pointer-events-none opacity-50">
          <span className="text-xs tracking-widest uppercase text-parchment mb-2">{t("header.scroll")}</span>
        ...
        {activeData && (
          <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 md:p-12 pointer-events-none">
            ...
            <motion.div 
              ...
              className="relative z-10 w-full max-w-6xl h-full md:h-[60vh] bg-ink rounded-2xl md:rounded-[2rem] overflow-hidden border border-gold/30 shadow-2xl flex flex-col md:flex-row pointer-events-auto"
            >
              {/* Close Button */}
              <button 
                onClick={() => setActiveCase(null)}
              ...
              <div className="w-full md:w-1/2 h-full flex flex-col justify-center p-8 md:p-16 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                ...
                <h2 className={`${locale === "ml" ? "text-2xl md:text-4xl leading-snug" : "text-3xl md:text-5xl leading-tight"} font-serif text-parchment mb-8 break-words`}>
                  {activeData.title}
                </h2>
                ...
                <div className="space-y-8 pb-12 md:pb-0">
                  <div>
                    ...
                    <p className="text-parchment/80 font-sans leading-relaxed md:text-lg">{activeData.detailedBrief}</p>
                  </div>
                </div>
              </div>
